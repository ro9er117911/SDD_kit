---
description: 從專案上下文產生資安需求與權限矩陣文件（70_infosec.md），包含資料分級、存取控制、身分驗證、日誌稽核與合規要求。
---
...
1. **載入相關文件**：
   ...
   - 設定 INFOSEC_DIR = `$REPO_ROOT/project/infosec`
...
8. **Q8: 適用法規與合規需求**
   ...
   **從專案性質推導**: [推導結果]
   範例：使用 Azure OpenAI → 必選「委外處理」、「新興科技」、「供應鏈風險」
...
**資安風險說明生成**：
- 從 10_business.md 和 20_process.md 判断是否涉及客戶交易
- 生成風險聲明：「本專案因未涉及客戶交易和往來，故無信用風險、市場風險、作業風險和其他風險等議題，先予敘明。」（若適用）
- 生成權限管理說明：
  - 從 10_business.md 推導權限開放範圍
  - 說明權限申請流程
  - 說明離職/職務異動的權限管理機制
- 生成網路安全說明：
  - 識別雲端服務使用（如 Azure OpenAI）
  - 說明內網/地端延伸架構
  - 說明定期弱點漏洞檢測
- 生成資料保護說明：
  - 浮水印機制
...
**法規檢視表生成**：
- 依據 Q8 選定的法規，生成詳細法規檢視表
- 每個法規包含：
  - 法規名稱
  - 主管機關
  - 條文重點（針對該法規的核心要求）
  - 本專案合規說明（從專案實際功能推導對應措施）
- 智能推導合規措施：
  - 若有 AI 模型 → 生成「模型使用日誌追蹤」、「模型版本管理」等措施
  - 若有個資處理 → 生成「個資欄位自動遮罩」、「存取權限控制」
  - 若使用 Azure 服務 → 生成「雲端服務風險評估」、「API 配額告警」
  - 若有開源套件 → 生成「White Source 掃描」、「套件授權管理」
- 生成法規遵循檢核週期：
  - 即時檢核、每月檢核、每季檢核、每年檢核
...
📄 檔案路徑: project/infosec/70_infosec.md
...
- **10_business.md** 的角色 → **70_infosec.md** 的權限矩陣
- **20_process.md** 的資料流 → **70_infosec.md** 的資料分級與加密點
- **30_risk_control.md** 的資安風險 → **70_infosec.md** 的技術控制措施
