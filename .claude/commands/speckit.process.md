---
description: 從使用者描述產生 As-Is / To-Be 流程文件（20_process.md），包含流程圖、例外處理、邊界情況、資料流動與整合點。
---
...
3. **檢查既有文件**：
   - 若 `$PROCESS_DIR/20_process.md` 已存在，詢問：
     - 「發現既有 20_process.md，是否要更新？(yes/no/merge)」
     - `yes`: 完全覆寫
     - `no`: 中止執行
...
```
💡 20_process.md 已生成，下一步建議：

選項 A：直接產生技術規格
  → 執行 `/speckit.specify --from-business-and-process`
  → 系統將整合 10_business.md 和 20_process.md 產生 spec.md
...
- **20_process.md 定義「怎麼流動」**（流程、協作）
- 使用者故事 → 對應流程步驟
- 業務痛點 → 對應流程瓶頸

### 與 /speckit.specify 的關係

- **20_process.md 為流程層需求**
- **spec.md 為功能層規格**
- 流程步驟 → 功能需求
- 例外處理 → 邊界情況
- RACI → 權限需求
