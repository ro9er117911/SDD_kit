...
2. **時程規劃**（若僅提供上線日期）：
   ...
   **細節版時程（系統/服務層級）**：
   - 從「涉及系統範圍」(Q5) 和「利害關係人」(Q8) 推導
   - 為每個系統/服務生成對應的功能項目與配合單位
   - 範例推導邏輯：
     - 系統：Azure Function → 服務：資料串接、ETL → 功能：API串接開發
     - 配合單位：從利害關係人中推導相關單位
   - 若資訊不足，標註 `[TBD - 請補充系統與功能細節]`
   
   **推導規則**：
   - 若專案類型為「系統整合」→ 增加「合規檢視」和「架構審議」階段
   - 若優先級為 P0/P1 → 壓縮各階段時程（快速通道）
   - 若涉及監理/法遵 → 延長「合規檢視」階段
...
3. **RACI 矩陣骨架（層級架構）**：
   - **智能推導三層組織架構**：
     ...
     **第一層（督導）**：
     - 自動設定「專案督導主管」為最高決策者（A）
     
     **第二層（指導/諮詢）**：
     - 從利害關係人單位（Q8）推導指導委員會成員
     - 若提及「金控」→ 生成「金控 XX 處」和「金控數位科技處」指揮鏈
     - 若提及「銀行」→ 生成「銀行資訊處」指揮鏈
     - 指導委員角色為 C (Consulted) 和 I (Informed)
     
     **第三層（執行）**：
     - 根據專案類型和涉及系統自動推導執行團隊：
       - **金控 XX 處團隊**：專案經理(R/A)、業務分析師(R)、業務需求開發(R)
       - **金控數位科技處團隊**（若涉及 AI/GAI）：
         * GAI 技術負責人(R/A)
         * GAI 模型開發工程師(R)
         * 資料科學家(R)
         * MLOps 工程師(R)
       - **銀行資訊處團隊**：
         ...
         * 系統整合工程師(R)
         * 測試工程師(R)
       - **跨職能支援**：資安(C)、法遵(C)、稽核(C)、使用者代表(R)
     
   - **組織架構圖自動生成**：
     - 根據識別的指揮鏈生成 ASCII 組織架構圖
     - 標註三層關係：督導 → 指導 → 執行
   
   - **科別/職稱欄位**：
     - 執行層表格包含「科別/職稱」欄位，支援跨科協作標註
     - 範例：「創新金融科/資深工程師」、「風控科/系統分析師」
...
3. **內容品質檢查**：
   ...
   - **清晰度檢查**：
     - [ ] 專案目標具體可理解
     - [ ] 時程規劃包含簡略版（階段層級）和細節版（系統層級）
     - [ ] 簡略版時程至少有 3 個專案階段
     - [ ] 細節版時程至少有 2 個系統/服務項目（若 Q5 有提供系統範圍）
     - [ ] 效益描述具體（避免過於空泛）
